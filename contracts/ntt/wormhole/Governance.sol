...
contract Governance {
    ...
    function _getConsumedGovernanceActionsStorage() private pure returns (mapping(bytes32 => bool) storage $) {
    ...
    function performGovernance(bytes calldata vaa) external {
        ...
        GeneralPurposeGovernanceMessage memory message = parseGeneralPurposeGovernanceMessage(verified.payload);
    ...
    function _verifyGovernanceVAA(bytes memory encodedVM) internal returns (IWormhole.VM memory parsedVM) {

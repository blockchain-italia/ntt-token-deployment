...
export async function deployHubNttManagerProxy(
  ...
  const hubManagerProxyContract = await new ERC1967Proxy__factory(signer).deploy(
    deployedNttHub.nttManagerImplementation,
    encodedCall
  );
...
export async function deploySpokeNttManagersProxy(
  ...
  for (const chain of chainsToDeploy) {
    ...
    const spokeNttManagerProxyContract = await new ERC1967Proxy__factory(signer).deploy(
      deployedSpokeNttContract.nttManagerImplementation,
      encodedCall
    );
...
export async function deployHubNttTransceiver(
  ...
  const wormholeTransceiverContract = await new WormholeTransceiver__factory(
    {
      ["contracts/ntt/libraries/TransceiverStructs.sol:TransceiverStructs"]:
        deployedNttContracts.hub.nttTransceiverStructs,
    },
    signer
  ).deploy(
...
export async function deploySpokeNttTransceivers(
  ...
  for (const chain of chainsToDeploy) {
    ...
    const wormholeSpokeTransceiverContract = await new WormholeTransceiver__factory(
      {
        ["contracts/ntt/libraries/TransceiverStructs.sol:TransceiverStructs"]:
          deployedSpokeNttContract.nttTransceiverStructs,
      },
      signer
    ).deploy(
...
export async function deployHubNttTransceiverProxy(
  ...
  const hubTrancieverProxyContract = await new ERC1967Proxy__factory(signer).deploy(
    deployedNttHub.nttTransceiverImplementation,
    encodedCall
  );
...
export async function deploySpokeNttTransceiversProxy(
  ...
  for (const chain of chainsToDeploy) {
    ...
    const spokeNttTrancieverProxyContract = await new ERC1967Proxy__factory(signer).deploy(
      deployedSpokeNttContract.nttTransceiverImplementation,
      encodedCall
    );
